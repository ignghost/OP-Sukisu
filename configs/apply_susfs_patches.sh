...
"$target" || true
  ...
  # to the 3-arg ksu_handle_setresuid(uid_t ruid, uid_t euid, uid_t suid)
  # (only the first arg, ruid, is actually consumed; the handler gates on the zygote SID).
  # SukiSU's tracepoint bridge still emits the OLD 2-arg call, which no longer compiles.
  # Rewrite the bridge call to the 3-arg form using the post-syscall creds. Guarded on the
  # 3-arg definition actually being present, so this stays a no-op on SUSFS v2.1.0.
  if grep -RqsE 'int[[:space:]]+ksu_handle_setresuid[[:space:]]*\([^)]*\beuid\b[^)]*\bsuid\b' \
       "$(dirname "$(dirname "$target")")" --include='*.c'; then
    echo "  setresuid: migrating bridge call to 3-arg v2.2.0 signature"
    perl -0pi -e 's/ksu_handle_setresuid\s*\(\s*old_uid\s*,\s*current_uid\(\)\.val\s*\)/ksu_handle_setresuid(current_uid().val, current_euid().val, current_uid().val)/g' "$target" || true
    # The pre-syscall old_uid local is now unused; keep it but silence -Wunused.
    perl -0pi -e 's/\buid_t\s+old_uid\s*=\s*current_uid\(\)\.val;/uid_t old_uid __maybe_unused = current_uid().val;/g' "$target" || true

    if grep -qE 'ksu_handle_setresuid\s*\(\s*old_uid\s*,' "$target"; then
      echo "::error::Old 2-arg ksu_handle_setresuid bridge call remains in $target"
      grep -n 'ksu_handle_setresuid' "$target" || true
      exit 1
    fi
  fi
...
return 0
  ...
  # Linux 6.1 / OP13R: the VFS handler takes struct filename **, while the
  # syscall-event bridge has a userspace filename pointer. Never pass the
  # userspace pointer to the VFS handler.
  if [ "$has_new_stat_filename" = true ] && [ "$has_new_stat_user" = false ] && \
     ! grep -qE '^[[:space:]]*int[[:space:]]+ksu_handle_stat_user[[:space:]]*\(' "$sucompat_c"; then
    cat >> "$sucompat_c" <<'OP13R_STAT_USER_EOF'

/* OP13R/Linux 6.1: userspace stat compatibility handler. */
int ksu_handle_stat_user(int *dfd, const char __user **filename_user, int *flags)
{
...

    {
        ...
        if (strncpy_from_user(path, *filename_user, sizeof(path) - 1) < 0)
            return 0;
...
OP13R_STAT_USER_EOF
    echo "  stat: added OP13R ksu_handle_stat_user() bridge handler"
    has_new_stat_user=true
  fi

  if [ "$has_old_stat" = true ]; then
    echo "  stat: keeping old ksu_handle_stat_sucompat bridge API"
  elif [ "$has_new_stat_user" = true ]; then
    perl -0pi -e 's/ksu_handle_stat_sucompat\s*\(\s*orig_nr\s*,\s*\(struct\s+pt_regs\s*\*\)\s*regs\s*\)/ksu_handle_stat_user((int *)\&PT_REGS_PARM1(regs), (const char __user **)\&PT_REGS_PARM2(regs), (int *)\&PT_REGS_SYSCALL_PARM4(regs))/g' "$target" || true
    perl -0pi -e 's/ksu_handle_stat\s*\(\s*dfd\s*,\s*filename_user\s*,\s*flags\s*\)/ksu_handle_stat_user(dfd, filename_user, flags)/g' "$target" || true
    echo "  stat: normalized syscall bridge to ksu_handle_stat_user()"
  elif grep -q 'ksu_handle_stat_sucompat' "$target"; then
    echo "::warning::stat bridge uses legacy ksu_handle_stat_sucompat() and no compatible replacement was found"
  fi
...
if [ -f kernel/Kbuild.rej ]; then
  # This SukiSU tree legitimately rejects the SUSFS hunk that removes these objects.
  if grep -qF -- '-kernelsu-objs += hook/lsm_hook.o' kernel/Kbuild.rej && \
     grep -qF -- '-kernelsu-objs += hook/syscall_event_bridge.o' kernel/Kbuild.rej && \
     grep -qF -- '-kernelsu-objs += infra/symbol_resolver.o' kernel/Kbuild.rej; then
    echo "Removing expected SukiSU Kbuild reject: kernel/Kbuild.rej"
    rm -f kernel/Kbuild.rej
  elif grep -q 'CONFIG_KSU_X86_PATCH_SYSCALL_DISPATCHER' kernel/Kbuild; then
...
if grep -q 'ksu_handle_stat_user' "$bridge"; then
  if ! grep -qE '^[[:space:]]*int[[:space:]]+ksu_handle_stat_user[[:space:]]*\(' "$bridge_sucompat_c" 2>/dev/null; then
    echo "::error::Bridge calls ksu_handle_stat_user but implementation is missing: $bridge"
    exit 1
  fi
fi
